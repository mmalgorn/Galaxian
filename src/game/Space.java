...
public class Space extends JComponent implements KeyListener, MouseListener {
	...
	ImagePanel imgFond, imgTitre, imgGameOver, imgBJ, imgBJC, imgBS, imgBSC, imgBQ, imgBQC, imgBR, imgBRC, imgBM,
			imgBMC;
	...
	// Met en mouvements les �l�ments du jeu(missiles, ennemies et le
	// vaisseau principale)
		...
		if (m.isMissileEnnemy()) {
			...
			for (int i = Invaders.invaders.size() - 1; i >= 0; i--) {
				...
				if (m.collideWith(inv)) {
					inv.getDamage();
					if (Invaders.invaders.size() == 0) try {
						Game.win();
					} catch (InterruptedException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
	...
	/*
	 * Fonctions permettant de g�rer les actions sur les touches de clavier et
	 * la souris
	 */

	@Override
	// Fonction de gestion des �v�nements des appuie sur une touche du
	// clavier
	public void keyPressed(KeyEvent e) {
		...
		case KeyEvent.VK_LEFT:
			if (!moveLeft && !pause && state == State.inGame) {
				tv.arret();
				tv.setDir("left");
				tv.reset();
				moveLeft = true;
		...
		case KeyEvent.VK_RIGHT:
			if (!moveRight && !pause && state == State.inGame) {
				tv.arret();
				tv.setDir("right");
				tv.reset();
				moveRight = true;
		...
		case KeyEvent.VK_SPACE:
			if (!fire && !pause && state == State.inGame) {
				Defender.def.fire();
	...
	@Override
	// Fonction de gestion des �v�nements de la relache sur une touche du
	// clavier
	...
	@Override
	// Fonction de gestion des �v�nements des appuies sur un clic de la
	// souris
	...
	@Override
	// Fonction de gestion des �v�nements des relaches sur un clic de la
	// souris
