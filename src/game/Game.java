		...
		if (nbLvl < 5) {
			if (!(Level.levelMap.get(nbLvl).boss.equals("null"))) {
				...
				for (int i = nbEnemyTmp; i < nbEnemyTmp + (Level.levelMap.get(nbLvl).nbShield); i++, nbEnemy++) {
					new ShieldInvaders(new Point(100 + ((i % 12) * 50), 100 + (i / 12) * 50), 2 + dif,
							Invaders.team.red);
				}
				nbEnemyTmp = nbEnemy;
			...
			if (!(Level.levelMap.get(nbLvl).boss.equals("null"))) {
				...
				for (int i = 0; i < (Level.levelMap.get(nbLvl).nbFire); i++, nbEnemy++) {
					new FireInvaders(new Point(100 + ((i % 12) * 50), 100 + (i / 12) * 50), 4 + dif,
							Invaders.team.yellow);
				}
				int nbEnemyTmp = nbEnemy;
				for (int i = nbEnemyTmp; i < nbEnemyTmp + (Level.levelMap.get(nbLvl).nbShield); i++, nbEnemy++) {
					new ShieldInvaders(new Point(100 + ((i % 12) * 50), 100 + (i / 12) * 50), 4 + dif,
							Invaders.team.yellow);
				}
				nbEnemyTmp = nbEnemy;
				for (int i = nbEnemyTmp; i < nbEnemyTmp + (Level.levelMap.get(nbLvl).nbPeon); i++, nbEnemy++) {
					new PeonInvaders(new Point(100 + ((i % 12) * 50), 100 + (i / 12) * 50), 4 + dif,
							Invaders.team.yellow);
				}
			}
		} else if (nbLvl < 15) {
			if (!(Level.levelMap.get(nbLvl).boss.equals("null"))) {
				...
				for (int i = 0; i < (Level.levelMap.get(nbLvl).nbFire); i++, nbEnemy++) {
					new FireInvaders(new Point(100 + ((i % 12) * 50), 100 + (i / 12) * 50), 6 + dif,
							Invaders.team.grey);
				}
				int nbEnemyTmp = nbEnemy;
				for (int i = nbEnemyTmp; i < nbEnemyTmp + (Level.levelMap.get(nbLvl).nbShield); i++, nbEnemy++) {
					new ShieldInvaders(new Point(100 + ((i % 12) * 50), 100 + (i / 12) * 50), 6 + dif,
							Invaders.team.grey);
				}
				nbEnemyTmp = nbEnemy;
				for (int i = nbEnemyTmp; i < nbEnemyTmp + (Level.levelMap.get(nbLvl).nbPeon); i++, nbEnemy++) {
					new PeonInvaders(new Point(100 + ((i % 12) * 50), 100 + (i / 12) * 50), 6 + dif,
							Invaders.team.grey);
				}
			}
		}
